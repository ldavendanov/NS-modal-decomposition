function [ym,Am,omega,theta,error,logMarginal,covariances] = DiagonalSS1cov(y,M,variances)
%--------------------------------------------------------------------------
% Estimation of the modal components of a non-stationary signal using the
% Joint EKF method 
% Created by : David Avendano - February 2019
%--------------------------------------------------------------------------
...
for i=2:N
    ...
    xi = xttm(:,i) - F*xttm(:,i-1);
    Cxi(:,:,i) = (i-1)/i*Cxi(:,:,i-1) + (1/i)*(xi*xi');
    Cv(:,:,i) = Qo;
%     Cv(:,:,i) = h*R*h' - F*h*R*h'*F';    
    Q = Cxi(:,:,i) + Cv(:,:,i);
...
Am = zeros(M,N);
for m=1:M
    omega(m,:) = abs(atan2(theta(2*m,:),theta(2*m-1,:)));
